...
let plus x y = x + y
let times x y = x * y
let simpler_sum xs = List.fold xs ~init:0 ~f:plus
let simpler_product xs = List.fold xs ~init:1 ~f:times
...
let plus_one from x = x + from

let range from to_ = List.init (to_ - from) ~f:(plus_one from)
(** ========== [List.range] ========== **)
...
let print x = Stdio.printf "%d" x
let print_int_list xs = List.iter xs ~f:print
